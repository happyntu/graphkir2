...
and `KIR2DS5` gains on difficult panels. The fallback alone still inherited one
5-digit KIR2DL1 miss from discard; the current lead fixes that miss with a
narrow discard-evidence suballele guard rather than broadening the fallback.
Do not promote this path to real-data defaults until it has real-data sanity
coverage.

The current synthetic lead is
`enhancedgate_kir2dl5_kir2ds5unsupported_kir2ds3rankwide_kir2dl1suballele_geneaware`. It keeps
the `KIR2DL1` functional fallback, narrow promotion guards for `KIR2DS5` and
`KIR2DS3`, the KIR2DL5-only unsupported-overcall guard, the targeted KIR2DS5
selected-allele unsupported-overcall guard, and adds a second rank-wide targeted
KIR2DS3 guard plus a KIR2DL1 discard-evidence suballele guard:
...
non-target allele to remain fixed. The KIR2DS3 rank-wide guard is deliberately
narrower than the prior `KIR2DS3*00201` experiment: it triggers only for exact
selected `KIR2DS3*0020101`, requires very poor selected private support, and
preserves the non-target 3-digit class. The exact-allele trigger is important:
using `KIR2DS3*00201` also touched true `KIR2DS3*0020103` in
`synthetic-difficult5x12` sample `08`.
The KIR2DL1 discard-evidence guard is also deliberately narrow: it triggers only
when the selected call contains `KIR2DL1*00303`, uses discard-style negative
evidence against unsupported candidate-only variants, and requires alternatives
to preserve the selected 3-digit genotype multiset. This fixed the
`synthetic-functional8x6` sample `05` 5-digit miss without allowing an unsafe
`KIR2DL1*025` 3-digit shift.
...
* `enhancedgate_kir2dl5_kir2ds5unsupported_kir2ds3rankwide_geneaware`: `1.0000 / 0.9973 / 0.9580`
* `enhancedgate_kir2dl5_kir2ds5unsupported_kir2ds3rankwide_kir2dl1suballele_geneaware`: `1.0000 / 0.9988 / 0.9595`
...
3/5-digit functional rows are now limited to KIR2DS3:

* KIR2DS3: one 5-digit all-methods-shifted row in seed5102 sample `00`

KIR2DL1, KIR2DS5, and KIR2DL5 have no remaining 3-digit or 5-digit functional
rows in the current stress sweep. KIR2DS3 is now 3-digit perfect on this stress
sweep.
Do not broaden the KIR2DS3/KIR2DS5 ambiguity handling without a new
sample-level audit showing a regression-safe target.

KIR2DL1-specific remaining-error audit is documented in
`docs/research/2026-05-11_kir2dl1_failure_audit.md` and generated by
`benchmarks/scripts/inspect_kir2dl1_remaining_failures.py`. The prior remaining
KIR2DL1 row was `synthetic-functional8x6` sample `05`, a 5-digit-only
`shared_with_discard` suballele miss under the fallback-only lead:

* truth `KIR2DL1*0030229_KIR2DL1*0030242`
* fallback/discard `KIR2DL1*0030242_KIR2DL1*00303`
* likelihood `KIR2DL1*0020102_KIR2DL1*0030205`

The KIR2DL1 functional fallback fixed the 3-digit class but inherited discard's
5-digit `00303` substitution. Variant evidence is truth-favoring overall in
both evidence modes: likelihood margin `339.000`, discard margin `286.000`.
However, the truth-only `KIR2DL1*0030229` marker `hv61` is itself unsupported
in likelihood and negative-leaning in discard, while `KIR2DL1*00303` has many
unsupported candidate-only variants. The current KIR2DL1 suballele guard uses
that evidence only as a narrow unsupported-overcall switch and requires the
selected 3-digit genotype multiset to be unchanged. Current KIR2DL1 audit output
therefore has no remaining 3/5-digit rows.
...
`benchmarks/scripts/inspect_kir2ds3_remaining_failures.py`. Under the current
rankwide candidate it finds one remaining KIR2DS3 5-digit sample:
...
Do not add a broad KIR2DS3 rollback or generic `KIR2DS3*002` guard from the
remaining seed5102 row. Leave it unchanged unless new evidence shows a safe way
to rescue `00113`.

The focused KIR2DS3 rank-failure audit is documented in
`docs/research/2026-05-11_kir2ds3_rank_failure.md` and generated by
`benchmarks/scripts/inspect_kir2ds3_rank_failure.py`. It confirms sample `03`
was not a near-window rerank problem before the rankwide guard:

* likelihood rank 1 is `KIR2DS3*0020101_KIR2DS3*011`, and discard/current/
  likelihood all agree on that call
* the true `KIR2DS3*011_KIR2DS3*016` call is rank `692`, gap `383.156`
* private-support score already favors truth over selected (`4.400` vs
  `-126.950`), but the likelihood gap is too large for the current top-window
  unsupported-overcall guard to see the truth
* selected-vs-truth discriminating variants strongly favor truth: one truth-only
  exon variant, 38 selected-only variants, five selected-only unsupported
  variants, and truth-selected net margin `964.000`

The rankwide `KIR2DS3*0020101` guard fixes this sample and improves
`synthetic-difficult5x12` from `0.9917 / 0.9917 / 0.9583` to
`1.0000 / 1.0000 / 0.9667` without changing the seed5102 row. Do not broaden it
to `KIR2DS3*00201`; that regressed `synthetic-difficult5x12` sample `08`, where
the true allele was `KIR2DS3*0020103` and the selected private-support score was
strongly positive.
