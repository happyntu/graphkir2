...
The current synthetic lead is `enhancedgate_kir2dl5guard_geneaware`. It keeps
the `KIR2DL1` functional fallback, narrow promotion guards for `KIR2DS5` and
`KIR2DS3`, and adds a KIR2DL5-only unsupported-overcall guard:
...
candidate regression during tuning.
...
candidate, the audit finds four KIR2DS5 remaining samples:

* 2 `shared_with_discard`
* 2 `unresolved_likelihood_pattern`
* 0 `candidate_regression`

All four have truth-favoring discriminating-variant margins, but the signal is
not a simple truth-only positive-rescue pattern. The strongest recurrent signal
is that candidate-only `KIR2DS5*027` evidence is unsupported: the selected
`KIR2DS5*0270102` exon variant `hv1797` has zero positive support and strong
negative support in all four remaining samples. Candidate-only unsupported
variants appear in six rows total; four are carried by selected `KIR2DS5*027`.
Do not broaden the existing KIR2DS5 promotion guard into a generic rollback.
The next KIR2DS5 method experiment should instead test a narrow
unsupported-overcall guard for selected `KIR2DS5*027`, using the same
sample-level variant-margin framing and verifying that `KIR2DS3` and `KIR2DS4`
gains remain intact.
