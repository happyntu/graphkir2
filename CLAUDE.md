...
`benchmarks/scripts/inspect_kir2dl5_remaining_failures.py`. It shows:

* one `ab_assignment_mismatch_on_merged_backbone` case where merged KIR2DL5 CN
  is correct (`2`) but one truth `KIR2DL5A` copy is placed as `KIR2DL5B`
* two `shared_allele_substitution` cases where KIR2DL5A copy count is correct
  but `KIR2DL5A*001` is selected as `KIR2DL5A*012`
* all KIR2DL5 truths remain inside current top-n (`rank 3`, `15`, `11`), so the
  blocker is likelihood/reranking rather than search truncation
* private-support evidence is mixed, favoring truth for two cases but the
  candidate for one case, so a KIR2DL5 rescue must be conditional rather than a
  broad global private-support switch

Keep `KIR2DL5A/B` separate from the KIR2DS3/KIR2DS5 guard work. If improving
KIR2DL5 next, prototype a narrow KIR2DL5A/B placement or KIR2DL5A*001-vs-*012
reranking experiment and re-run the full functional stress sweep before touching
the current KIR2DS3/KIR2DS5 gains. KIR2DL1 3-digit is fixed; the remaining
KIR2DL1 issue is one shared 5-digit suballele miss.
...
`benchmarks/scripts/inspect_kir2dl5_discriminating_variants.py`. For all three
remaining KIR2DL5 cases, the truth genotype has no truth-only variants relative
to the wrong candidate, while the wrong candidate introduces candidate-only
variants that are unsupported or contradicted by negative reads:

* sample 09: `KIR2DL5B*01301` adds 17 candidate-only variants, net `-297.000`
* seed5101 sample 11: `KIR2DL5A*01201` adds 10 candidate-only variants, net
  `-49.000`
* seed5103 sample 04: `KIR2DL5A*01201` adds 10 candidate-only variants, net
  `-43.000`; one ambiguous positive exon signal exists, but negative evidence
  still favors truth

The next KIR2DL5 method experiment should therefore penalize unsupported
candidate-only variants for KIR2DL5A/B placement and KIR2DL5A*001-vs-*012
overcalls. Do not implement a broad KIR2DL5 private-positive rescue first.
