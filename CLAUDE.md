...
`enhancedgate_kir2dl5_kir2ds5unsupported_kir2ds3rankwide_geneaware`. It keeps
...
current stress sweep. KIR2DS3 is now 3-digit perfect on this stress sweep.
...
`benchmarks/scripts/inspect_kir2dl1_remaining_failures.py`. The only remaining
KIR2DL1 row is `synthetic-functional8x6` sample `05`, a 5-digit-only
`shared_with_discard` suballele miss:

* truth `KIR2DL1*0030229_KIR2DL1*0030242`
* current/discard `KIR2DL1*0030242_KIR2DL1*00303`
...
unsupported candidate-only variants. A KIR2DL1 suballele method should therefore
be framed as a narrow unsupported-overcall/tie-break experiment that preserves
the `KIR2DL1*003` 3-digit class, not as a broad discard rollback.
