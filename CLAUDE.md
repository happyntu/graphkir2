...
is not a near-window rerank problem:
...
Next KIR2DS3 method work should therefore test a narrow pre-likelihood or
early-post-likelihood `KIR2DS3*0020101` overcall penalty/pruning rule that uses
selected-only unsupported evidence and preserves the shared non-target allele
class. Do not broaden this into a generic `KIR2DS3*002` rollback: seed5102
sample `00` still has truth-only `KIR2DS3*0011302` evidence that is unsupported,
so any rescue must be sample-evidence gated and must preserve the current
KIR2DS3/KIR2DS4/KIR2DS5 gains.
