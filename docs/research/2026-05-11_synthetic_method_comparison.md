...
| synthetic-difficult5 | functional-target+kir2ds3-private | 1 | 1 | 0.9 |
...
reranking plus `KIR2DS3/KIR2DS5` cross-gene neutralization. It preserves
`synthetic-functional8` and `synthetic-functional8x6` at the same `3/5-digit`
scores as the top5000 baseline and raises `synthetic-difficult5` to
`3/5-digit = 1.0`, but trades `synthetic-difficult5` `7-digit` from
`0.925` to `0.9`. Treat it as the current functional target, not the
balanced default.
